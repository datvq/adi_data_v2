...
def output_file_default() -> str:
  return os.path.join(shared.env.data_dir, "staged", "bravo", "flat_inventory_lot.parquet")

def df_fact_inventory_lot_default() -> str:
  data_file = os.path.join(shared.env.data_dir, "staged", "bravo", "fact_inventory_lot.parquet")
...
def df_dim_lot_default() -> str:
  data_file = os.path.join(shared.env.data_dir, "staged", "bravo", "dim_lot.parquet")
  return dx.df.read_data_file(data_file)

def execute(df_fact_inventory_lot: pl.DataFrame, df_dim_date: pl.DataFrame, df_dim_warehouse: pl.DataFrame, df_dim_item: pl.DataFrame, df_dim_lot: pl.DataFrame) -> pl.DataFrame:
  # return
  df_flat_inventory_lot: pl.DataFrame

  # context
  ctx_dwh: pl.SQLContext = pl.SQLContext()
  
  ctx_dwh.register("fact_inventory_lot", df_fact_inventory_lot)
  ...
  ctx_dwh.register("dim_lot", df_dim_lot)
  
  ctx_dwh.tables()

  # transform
  query_flat_inventory_lot = """
  select
    fact_inventory_lot.view_date view_date,
    ...
    dim_date.yyqq yyqq,
    dim_date.yymm yymm,
    fact_inventory_lot.warehouse_code warehouse_code,
    ...
    fact_inventory_lot.item_code item_code,
    ...
    fact_inventory_lot.item_lot_code item_lot_code,
    fact_inventory_lot.lot_code lot_code,
    dim_lot.mfg_date mfg_date,
    dim_lot.exp_date exp_date,
    fact_inventory_lot.quantity quantity,
    (fact_inventory_lot.quantity * dim_item.net_weight) weight,
    (fact_inventory_lot.quantity / nullif(dim_item.units_per_package, 0)) packages,
  from fact_inventory_lot fact_inventory_lot
    left join dim_date dim_date on fact_inventory_lot.view_date = dim_date.date
    left join dim_warehouse dim_warehouse on fact_inventory_lot.warehouse_code = dim_warehouse.warehouse_code
    left join dim_lot dim_lot on fact_inventory_lot.lot_code = dim_lot.lot_code
    left join dim_item dim_item on fact_inventory_lot.item_code = dim_branch.item_code
  """
  df_flat_inventory_lot = ctx_dwh.execute(query=query_flat_inventory_lot, eager=True)

  return df_flat_inventory_lot
